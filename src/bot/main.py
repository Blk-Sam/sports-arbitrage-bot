...
class ManualPnLAnalyzer:
    ...
    def load_manual_pnl(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"✅ Loaded {len(df)} manual bet records from {self.manual_pnl_file}")
            return df
    ...
    def analyze_manual_data(self) -> Dict[str, Any]:
        ...
        logger.info(f"📊 Manual P&L Insights:")
    ...
    def get_adjusted_min_margin(self, base_margin: Decimal) -> Decimal:
        ...
        if win_rate < 40:
            ...
            logger.info(f"⚠️ Low win rate ({win_rate:.1f}%) - increasing margin to {adjusted:.4f}")
            return adjusted
        elif win_rate < 50:
            adjusted = base_margin * Decimal("1.2")
            logger.info(f"📊 Moderate win rate ({win_rate:.1f}%) - increasing margin to {adjusted:.4f}")
            return adjusted
        else:
            logger.info(f"✅ Good win rate ({win_rate:.1f}%) - keeping margin at {base_margin:.4f}")
            return base_margin
    ...
    def get_current_bankroll(self, default: Decimal) -> Decimal:
        ...
        try:
            ...
            logger.info(f"💰 Using manual P&L bankroll: ${latest_bankroll:.2f}")
...
async def process_arbitrage_with_notifications(
    ...
    logger.info(f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logger.info(f"🎯 Processing arbitrage {arb_id}")
    logger.info(f"📊 {total_bets} bets | Expected profit: ${profit:.2f}")
    logger.info(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    for idx, team in enumerate(team_names):
        ...
        logger.info(f"📍 [{idx+1}/{total_bets}] Placing bet: ${stake:.2f} on {team} @ {bm}")
        ...
        if not success:
            logger.error(f"❌ Failed to place bet {idx+1}/{total_bets}")
        ...
        logger.info(f"✅ [{idx+1}/{total_bets}] Bet placed successfully")
    ...
    if len(placed_bets) == total_bets:
        ...
        logger.info(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        logger.info(f"🎉 All {total_bets} bets placed for {arb_id}")
        logger.info(f"💰 Guaranteed profit: ${profit:.2f} ({roi:.2f}% ROI)")
        logger.info(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
async def main():
    ...
    try:
        ...
        if backup_path:
            ...
            logger.info(f"✅ Startup backup created: {backup_path} ({backup_size_mb:.2f} MB)")
            send_backup_notification(backup_path, "startup", backup_size_mb)
        else:
            logger.warning("⚠️ Startup backup failed (non-critical)")
    except Exception as e:
        logger.warning(f"⚠️ Startup backup error (non-critical): {e}")
    
    logger.info("=" * 70)
    
    # === MANUAL P&L INTEGRATION ===
    logger.info("\n" + "=" * 70)
    logger.info("📊 LOADING MANUAL P&L DATA FOR ADAPTIVE LEARNING")
    logger.info("=" * 70)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("⚙️  INITIALIZING ADAPTIVE POLLER & RATE LIMITER")
    logger.info("=" * 70)
    ...
    logger.info(f"📊 Adaptive Polling Configuration:")
    ...
    try:
        ...
        for idx, sport in enumerate(prioritized_sports):
            ...
            if not manual_pnl_analyzer.should_bet_on_sport(sport):
                logger.warning(f"⏭️  Skipping {sport} - unprofitable in manual P&L history")
                continue
            
            # Check if should poll this sport based on quota
            if not adaptive_poller.should_poll_sport(sport):
                logger.warning(f"⏭️  Skipping {sport} - quota preservation mode")
                arbitrage_skipped += 1
                continue
            ...
            logger.info(f"\n{'=' * 60}")
            logger.info(f"🕐 Scanning {sport} (Priority #{idx+1}) | Interval: {poll_interval}s")
            logger.info(f"{'=' * 60}")
            ...
            for game in odds_data:
                ...
                if not manual_pnl_analyzer.should_bet_on_market(best_arb['market']):
                    logger.warning(f"⏭️  Skipping {best_arb['market']} market - unprofitable in manual P&L")
                    arbitrage_skipped += 1
                    continue
                ...
                for i, team in enumerate(team_names):
                    logger.info(
                        ...
                        f"Bet on {team} at odds {odds[i]:.3f} — Stake: ${stakes[i]:.2f}"
                    )
            ...
            if idx < len(prioritized_sports) - 1:  # Don't sleep after last sport
                logger.info(f"⏱️  Adaptive sleep: {poll_interval}s before next sport")
            ...
            if collector.calls_made / collector.max_calls > 0.9:
                ...
                logger.warning("⚠️ Forcing extended sleep due to high quota usage")
        ...
        summary_msg = (
            f"✅ *Bot Run Complete*\n\n"
            f"📊 Arbitrages Found: {arbitrage_found}\n"
            f"⏭️  Skipped (Filters): {arbitrage_skipped}\n"
            f"💰 Profit: ${bankroll_stats['profit']:.2f}\n"
            f"📈 ROI: {bankroll_stats['roi']:.2f}%\n"
            f"🏦 Bankroll: ${bankroll_stats['current']:.2f}\n"
            f"📞 API Calls: {api_key_mgr.total_calls}\n"
            f"🧠 Manual Bets Learned: {manual_pnl_analyzer.insights['total_bets']}"
        )
        ...
        try:
            ...
            if backup_path:
                backup_size_mb = os.path.getsize(backup_path) / (1024 * 1024)
                logger.info(f"✅ Shutdown backup created: {backup_path} ({backup_size_mb:.2f} MB)")
                send_backup_notification(backup_path, "shutdown", backup_size_mb)
            else:
                logger.warning("⚠️ Shutdown backup failed (non-critical)")
        except Exception as e:
            logger.warning(f"⚠️ Shutdown backup error (non-critical): {e}")
