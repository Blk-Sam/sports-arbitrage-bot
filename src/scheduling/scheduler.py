...
def load_manual_pnl_analyzer():
    ...
    try:
        ...
        try:
            ...
            logger.warning("❌ ManualPnLAnalyzer not found in bot.main - adaptive poller will use default priorities")
            return None
        ...
        if not os.path.exists(manual_pnl_path):
            logger.info(f"ℹ️ Manual P&L file not found: {manual_pnl_path} - using default priorities")
            return None
        ...
        if hasattr(analyzer, 'insights') and isinstance(analyzer.insights, dict):
            ...
            logger.info(f"✅ Loaded manual P&L analyzer: {total_bets} historical bets")
            return analyzer
        else:
            logger.warning("⚠️ ManualPnLAnalyzer loaded but missing expected 'insights' attribute")
            return None
    
    except Exception as e:
        logger.warning(f"⚠️ Could not load manual P&L analyzer: {e}")
        return None
        ...
        if should_send:
            ...
            try:
                ...
                if backup_path:
                    ...
                    logger.info(f"✅ Daily backup created: {backup_path} ({backup_size_mb:.2f} MB)")
                    ...
                    logger.warning("⚠️ Daily backup failed (non-critical)")
...
def healthcheck_heartbeat() -> None:
    ...
    try:
        ...
        if _adaptive_poller and _adaptive_poller.is_sleep_hours():
            ...
            logger.info(f"[HEARTBEAT] 💤 Sleeping - Wake at {sleep_status['wake_time']}, API calls: {api_key_mgr.total_calls}")
        else:
            logger.info(f"[HEARTBEAT] ☀️ Active - Uptime: {uptime_hours:.1f}h, API calls: {api_key_mgr.total_calls}")
...
def should_run_now_adaptive() -> tuple:
    ...
    if _adaptive_poller.is_sleep_hours():
        sleep_status = _adaptive_poller.get_sleep_status()
        return False, f"💤 Sleep mode active until {sleep_status['wake_time']}"
...
def get_dynamic_sleep_interval() -> int:
    ...
    if ADAPTIVE_SCHEDULING and _adaptive_poller:
        ...
        if sports:
            ...
            logger.info(f"📊 Adaptive average interval: {avg_interval:.0f}s ({avg_interval/60:.1f} min)")
...
def run_bot_with_key(api_key: str) -> bool:
    ...
    if not should_run:
        logger.info(f"⏭️ Skipping bot run: {reason}")
    ...
    try:
        ...
        if result.returncode == 0:
            send_telegram_msg("✅ <b>Bot completed successfully</b>")
    ...
    for sport in sports_to_scan:
        ...
        if _adaptive_poller:
            ...
            if not _adaptive_poller.should_poll_sport(sport):
                logger.info(f"⏭️ Skipping {sport} - quota preservation")
                continue
...
def dynamic_scheduler() -> None:
    ...
    if ADAPTIVE_SCHEDULING:
        logger.info("\n⚙️ Initializing adaptive polling system...")
        ...
        logger.info(f"📊 Adaptive Polling Status:")
        ...
        try:
            ...
            if _adaptive_poller and _adaptive_poller.is_sleep_hours():
                if not _sleep_notified:
                    sleep_status = _adaptive_poller.get_sleep_status()
                    logger.info(f"💤 Entering sleep mode until {sleep_status['wake_time']}")
                ...
                if _sleep_notified:
                    logger.info("☀️ Waking up - resuming normal operations")
            ...
            if delay <= 0:
                ...
                send_telegram_msg(f"🎯 Running bot now for event at {next_event.strftime('%Y-%m-%d %H:%M:%S')} UTC")
                ...
                send_telegram_msg(
                    f"⏰ Next bot run in {delay/60:.1f} min for event at {next_event.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                )
